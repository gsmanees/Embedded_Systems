...
void OLED_PrintFloat_1Decimal(float value)
{
	...
	int_part = (int)value;
	dec_part = (int)((value - int_part) * 10);

	OLED_PrintNumber(int_part);
	OLED_PrintChar('.');
	OLED_PrintChar(dec_part + '0');
}

// temp_x10 = temperature × 10
// Example: 311 ? 31.1°C
void OLED_PrintTemp_1Decimal(int temp_x10)
{
	int tens, ones, frac;

	tens = temp_x10 / 100;           // 3
	ones = (temp_x10 / 10) % 10;     // 1
	frac = temp_x10 % 10;            // 1

	if (tens > 0)
	OLED_PrintChar(tens + '0');

	OLED_PrintChar(ones + '0');
	OLED_PrintChar('.');
	OLED_PrintChar(frac + '0');
}


// temp_x100 = temperature × 100
// Example: 2534 ? "25.34"
void OLED_PrintTemp_2Decimal(int temp_x100)
{
	int hundreds, tens, ones, d1, d2;
	...
	hundreds = temp_x100 / 1000;        // tens digit (if any)
	tens     = (temp_x100 / 100) % 10;  // ones digit
	ones     = (temp_x100 / 10) % 10;   // first decimal
	d2       = temp_x100 % 10;           // second decimal

	// Print integer part
	if (hundreds > 0)
	OLED_PrintChar(hundreds + '0');
